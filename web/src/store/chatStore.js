...
export default {
  ...
  mutations: {
    ...
    readedMessage(state, pos) {
      ...
      chat && this.commit('saveToStorage')
    },
    ...
    removePrivateChat(state, friendId) {
      let chats = this.getters.findChats()
      for (let idx in chats) {
        if (chats[idx].type == 'PRIVATE' && chats[idx].targetId === friendId) {
    ...
    insertMessage(state, msgInfo) {
      ...
      if (
        msgInfo.id &&
        type == 'PRIVATE' &&
        msgInfo.id > state.privateMsgMaxId
      ) {
      ...
      if(!chat) {
        console.log('insertMessage can not find chat lynn:',msgInfo)
        if(type === 'GROUP') {
          return;
        }
        // 暂时没有处理群组消息的问题
        ...
        const chats = this.getters.findChats() 
        chats.unshift(chat)
        this.commit('saveToStorage')
      }
  ...
  getters: {
    ...
    findChatByGroup: (state, getters) => (gid) => {
      let chats = getters.findChats()
      return chats.find((chat) => chat.type == 'GROUP' && chat.targetId == gid)
    },
