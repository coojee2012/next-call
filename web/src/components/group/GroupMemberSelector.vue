<template>
  <el-dialog title="选择成员" v-model="isShow" width="50%">
      ...
      <div class="left-box">
        ...
        <el-scrollbar style="height: 400px">
          <div v-for="m in members" :key="m.userId">
            <group-member-item
              v-show="!m.quit && m.showNickName.includes(searchText)"
              :member="m"
              @click="onClickMember(m)"
            >
              <el-checkbox
                :disabled="m.locked"
                v-model="m.checked"
...
export default {
  ...
  methods: {
    ...
    ok() {
      $emit(this, 'complete', this.checkedMembers)
      this.isShow = false
    },
    close() {
      this.isShow = false
