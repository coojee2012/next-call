  ...
  <el-container class="friend-page">
    <el-aside width="280px" class="friend-list-box">
      ...
      <el-scrollbar class="friend-list-items">
        ...
        >
          <friend-item
            v-show="friend.nickName && friend.nickName.includes(searchText)"
            :index="index"
...
export default {
  ...
  methods: {
    ...
    loadUserInfo(friend, index) {
      this.$http({
        url: `/users/find/${friend.friendId}`,
        method: 'get',
      }).then((user) => {
        this.userInfo = user
        // 如果发现好友的头像和昵称改了，进行更新
  ...
  computed: {
    ...
    isFriend() {
      return this.friendStore.friends.find((f) => f.friendId == this.userInfo.id)
    },
  },
}
</script>
