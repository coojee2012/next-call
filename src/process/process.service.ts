...
export class ProcessService {
	...
	async socketClientsPublish(process: IProcess): Promise<boolean> {
		...
		clients.forEach((client: IActivity) => {
			const activityResponse = "PROCESS_" + process.permission
			const processChallenge = ActivityResponse[activityResponse as ActivityResponse];
			...
			const serverResponse = [
				...
				ProcessPermissionMap[process.permission],
			];
	...
	async findOneById(id: number): Promise<IProcess | null> {
		return await this.processRepository.findOneBy({id});
	}
	...
	onMessage(dgramSocket: any) {
		...
		dgramSocket.on('message', async (msg:any, rinfo:any) => {
			try {
			...
			if (ActivityRequestMap[+requestType] === 'CHECK_HASH') {
				...
				const findProcess = await this.processRepository.findOneBy({
					hash,
				});
				...
				const c = ActivityResponseMap[processChallenge]
				const res = {
					guid,
					hostname,
					filename,
					...
					port: rinfo.port.toString(),
					from: 'SERVER',
					response: ActivityResponse[c as ActivityResponse],
				};
				...
				const b = ProcessPermissionMap[+value];
				const processPermissionMapped = ProcessPermission[b as ProcessPermission];
				...
				if (findProcess) {
					...
					const c1 = findProcess.adminHelp
					adminHelpResponse = ActivityResponseAdminHelpMap.ADMIN_HELP_END;
					const c2 = ActivityResponseAdminHelpMap[adminHelpResponse]
					activityAdminHelp = ActivityResponse[c2 as ActivityResponse];
